...
export class SecuencialFormPageComponent {
  ...
  constructor() {
    ...
    this.establecimientoId = this.route.snapshot.queryParamMap.get('establecimientoId') ?? '';
    this.puntoEmisionId = this.route.snapshot.queryParamMap.get('puntoEmisionId') ?? '';
    ...
    effect(() => {
      ...
      if (this.route.snapshot.queryParamMap.get('empresaId')) {
        return;
      }
  ...
  protected volver(): void {
    this.router.navigate(['/catalogos'], { queryParams: { tab: 'secuenciales' } });
  }
    ...
    this.secuencialesService
      ...
      .pipe(
        catchError(() => {
          ...
          this.alerts.error('Secuencial no guardado', 'No se pudo actualizar el secuencial seleccionado.');
          return of(null);
        }),
        finalize(() => {
          this.savingTipo = '';
      ...
      .subscribe((response) => {
        ...
        this.alerts.success('Secuencial actualizado', `Se guardo ${this.formatLabel(response.tipoDocumento)} correctamente.`);
      });
  }
  ...
  private cargarEmpresas(): void {
    this.empresasService
      ...
      .subscribe((response) => {
        ...
        if (this.empresaId) {
          this.cargarEstablecimientos(this.empresaId, this.establecimientoId);
  ...
  private cargarEstablecimientos(empresaId: string, selectedId = ''): void {
    this.establecimientosService
      ...
      .subscribe((response) => {
        ...
        this.establecimientoId = selectedId || this.establecimientos[0]?.id || '';
        if (this.establecimientoId) {
          this.cargarPuntosEmision(this.establecimientoId, this.puntoEmisionId);
        }
      });
  }

  private cargarPuntosEmision(establecimientoId: string, selectedId = ''): void {
    this.puntosEmisionService
      ...
      .subscribe((response) => {
        ...
        this.puntoEmisionId = selectedId || this.puntosEmision[0]?.id || '';
        if (this.puntoEmisionId) {
          this.cargarSecuenciales(this.puntoEmisionId);
        }
      });
